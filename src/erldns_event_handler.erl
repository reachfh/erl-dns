...
handle_event({end_udp, [{host, _Host}]}, State) ->
  metrics:update(udp_request_meter, {c, 1}),
  metrics:update(udp_request_counter),
  {ok, State};

handle_event({end_tcp, [{host, _Host}]}, State) ->
  metrics:update(tcp_request_meter, {c, 1}),
  metrics:update(tcp_request_counter),
  {ok, State};

handle_event({udp_error, Reason}, State) ->
  metrics:update(udp_error_meter, {c, 1}),
  folsom_metrics:notify({udp_error_history, Reason}),
  {ok, State};

handle_event({tcp_error, Reason}, State) ->
  metrics:update(tcp_error_meter, {c, 1}),
  folsom_metrics:notify({tcp_error_history, Reason}),
  {ok, State};

handle_event({refused_response, Questions}, State) ->
  metrics:update(refused_response_meter, {c, 1}),
  metrics:update(refused_response_counter),
  lager:debug("Refused response: ~p", [Questions]),
  {ok, State};

handle_event({empty_response, Message}, State) ->
  metrics:update(empty_response_meter, {c, 1}),
  metrics:update(empty_response_counter),
  lager:info("Empty response: ~p", [Message]),
  {ok, State};

handle_event({dnssec_request, _Host, _Qname}, State) ->
  metrics:update(dnssec_request_counter),
  metrics:update(dnssec_request_meter, {c, 1}),
  {ok, State};
